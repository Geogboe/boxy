...
`.boxy/server.crt`, `.boxy/server.key`. These files are entirely
boxy-managed — generated by `pkg/pki.EnsureCA`/`IssueServerCert`, and (see
below) regenerated automatically when the configured SANs change — not a
location for an operator-supplied cert/key pair.
...
Unlike `--insecure`, the server certificate's extra SANs
(`server.grpc_cert_sans` / `--grpc-cert-san`, repeatable) **are** safe to
expose as a config field: adding a SAN never weakens TLS or mTLS — a
connecting agent still must trust boxy's private CA and the server still
must hold the matching private key; a SAN only changes which
hostname/IP a client is permitted to present in its connection request to
be considered a match for hostname verification. This matters for remote
agents that reach the gRPC listener through a passthrough route or load
balancer under a public/external DNS name rather than the literal address
`--grpc-listen` binds to — the previous SAN set (`localhost`, `127.0.0.1`,
and the literal `--grpc-listen` host) had no way to include that name, so
hostname verification failed even though the agent correctly trusted the
CA. `IssueServerCert` detects when the configured SAN set no longer
matches the on-disk certificate's SANs (compared as an order-independent
set of DNS names/IPs) and regenerates it automatically on the next `boxy
serve` start — an operator changing this setting doesn't need to know to
delete `.boxy/server.crt`/`.boxy/server.key` manually. If the existing
cert file is corrupt/unparseable, `IssueServerCert` fails loudly instead of
silently regenerating — delete both files by hand and restart to recover.
Manual deletion remains the only way to rotate the CA itself, a separate,
out-of-scope concern here.
...
Hyper-V now requires a provider-level `providers[].config.memory_budget_mb`
limit for the combined startup memory of Boxy-managed VMs. Before reserving
memory, the provider retries the live Windows memory probe briefly and uses
the smaller of the configured budget remaining and live free memory after the
host reserve. This prevents separate pools from overcommitting one host while
still respecting memory used outside Boxy. The host reserve remains available
as `providers[].config.host_reserve_mb`, with a 512 MiB default and explicit
zero support. Both settings are available to embedded and remote agents. Both the
