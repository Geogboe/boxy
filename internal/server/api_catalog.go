...
func APIRouteCatalog() []APIRoute {
	return []APIRoute{
		...
		{Group: "Identity", Method: "GET", Path: "/api/v1/identity", Auth: "user/auditor/admin", Description: "Return the authenticated caller's own key ID, role, kind, and subject; the role-neutral check `boxy login` verifies a key against."},
		{Group: "Pools", Method: "GET", Path: "/api/v1/pools", Auth: "auditor/admin", Description: "List configured pools and ready inventory."},
		{Group: "Pools", Method: "GET", Path: "/api/v1/pools/summary", Auth: "user/auditor/admin", Description: "List pool name/type/profile only -- the safe discovery view a user-role caller needs to construct a sandbox request."},
		{Group: "Pools", Method: "GET", Path: "/api/v1/pools/{name}", Auth: "auditor/admin", Description: "Inspect one pool."},
		{Group: "Pools", Method: "PUT", Path: "/api/v1/pools/{name}/configuration", Auth: "admin", Description: "Validate, persist, and apply editable pool policy settings."},
		{Group: "Pools", Method: "POST", Path: "/api/v1/pools/{name}/drain", Auth: "admin", Description: "Start a tracked job to drain unused ready inventory."},
		{Group: "Pools", Method: "POST", Path: "/api/v1/pools/{name}/fill", Auth: "admin", Description: "Start a tracked job to reconcile a pool to its configured target."},
		...
		{Group: "Jobs", Method: "GET", Path: "/api/v1/jobs/{id}", Auth: "auditor/admin", Description: "Inspect durable job status and step progress."},
		{Group: "Jobs", Method: "POST", Path: "/api/v1/jobs/{id}/cancel", Auth: "admin", Description: "Request cancellation; completion follows cleanup."},
		...
		{Group: "Sandboxes", Method: "POST", Path: "/api/v1/sandboxes/{id}/exec", Auth: "user/admin", Description: "Queue one durable command execution and return its execution ID; command, command_text, and script are mutually exclusive."},
		{Group: "Sandboxes", Method: "GET", Path: "/api/v1/sandboxes/{id}/exec/{exec_id}", Auth: "user/admin", Description: "Read bounded output chunks and lifecycle status after an opaque cursor."},
		{Group: "Sandboxes", Method: "POST", Path: "/api/v1/sandboxes/{id}/exec/{exec_id}/cancel", Auth: "user/admin", Description: "Cancel a pending or running execution."},
		...
		{Group: "Agents", Method: "POST", Path: "/api/v1/agents/{id}/logs", Auth: "admin", Description: "Start a tracked job that pulls a bounded remote diagnostics snapshot."},
