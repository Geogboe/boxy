...
// BuildRegistry constructs a provider registry based on pools and available backends.
// It runs lightweight health checks (docker) and skips unavailable providers instead of failing.
func BuildRegistry(ctx context.Context, pools []pool.PoolConfig, logger *logrus.Logger, encryptor crypto.Encryptor) *provider.Registry {
	...
	if needsScratchShell(pools) {
		shCfg := scratchShellConfigFromPools(pools)
		registry.Register("scratch/shell", shell.New(logger, shCfg))
		logger.Info("scratch/shell provider registered")
	}

	// Hyper-V (only if requested and on Windows)
	if poolsNeedBackend(pools, "hyperv") && isWindows() {
...
func isWindows() bool {
	// Late import of runtime avoided in signature; keep function small.
	return runtimeGOOS == "windows"
}

var runtimeGOOS = getRuntimeGOOS()
