...
//
// What:
// - Provider is an external system Boxy can provision resources on. Examples:
//   a specific Docker engine, or a specific Hyper-V host.
//
// Why:
...
//
// When to use:
// - Use Provider as configured data in your domain model and persistence.
// - Use Provider.Type to select which driver/adapter implementation can talk to
//   the Provider.
//
// How to use correctly:
// - Treat Provider as configuration, not behavior. The driver/adapter layer is
//   where IO happens (Docker API calls, PowerShell, etc).
// - If you allow Provider renames, prefer referencing Providers by Provider.ID
//   internally for stability, while continuing to accept Provider.Name as the
//   user-facing handle in CLI/config.
// - Avoid using the word "provider" to mean both the code and the external
//   system. In this repo:
//   - Provider = external configured target
//   - Driver/Adapter = code that talks to a ProviderType
//
// ---
//
// # Pool
