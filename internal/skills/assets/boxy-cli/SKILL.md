...
- `boxy agent service install --server <host:port> --providers <list> --token <token> --ca-cert <path>` installs the agent as a background service instead of a foreground process. Pass the same `--config` flag to snapshot provider instances into the service configuration. It is a real service by default (requires an elevated process), or `--user` for an unprivileged Task Scheduler/systemd-user fallback. `boxy serve service install` does the same for the daemon. Pass `--instance-name <name>` to run more than one instance side by side on one host (`boxy-agent-<name>`/`boxy-serve-<name>`, distinct default data/state dirs); `uninstall`/`start`/`stop`/`status` need the same `--instance-name` (and `--user`, if that's how it was installed) to target that instance. `--enable-mesh-overlay` on a real (non-`--user`) install also grants the systemd service `CAP_NET_ADMIN` on Linux, since an unprivileged `--user` unit cannot be. See docs/service-install.md.
