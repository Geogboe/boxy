...
**REST API** — for CLI-to-server communication. Standard HTTP REST, always served alongside the web dashboard by `boxy serve` (no separate `api.enabled` gate). TLS (Boxy's own private CA by default) and bearer API-key auth with `user`/`auditor`/`admin` roles are on by default; see [ADR-0007](docs/adr/0007-secure-rest-api-and-cli-authentication.md).
...
Command execution is durable as well. `POST /api/v1/sandboxes/{id}/exec` returns
`202 Accepted` and an execution ID; clients read output and status from the
execution endpoint and resume with its opaque `next` cursor after a disconnect.
Output is capped at 1 MiB per execution, terminal records remain for 24 hours,
and a server restart marks active executions `interrupted` without replaying
them. Only one execution may use a resource at a time (`409 resource_busy`);
different resources can execute concurrently. The CLI maps this to live output,
`--detach`, `--attach`, `--events`, and `--buffered`.
...
When `server.oidc` is configured, the login page can use `login_label` and
`login_icon` to match the organization's SSO provider. Set
`hide_local_login: true` only when the OIDC provider is configured and the
local break-glass login is intentionally unavailable.
...
boxy sandbox exec <id> [-- <command> [args...]]
                                          — submit a durable execution with live output
                                            (`--command` and `--stdin` send opaque text;
                                             `--script-file` preserves script args)
                                          — `--detach` prints an execution ID;
                                            `--attach <exec-id>` reconnects
                                          — `--events` emits NDJSON; `--buffered` waits
                                            for terminal output
