...
- `POST /api/v1/sandboxes/{id}/exec` and `boxy sandbox exec` execute non-interactive commands only in ready sandboxes. Single-resource sandboxes select implicitly; multi-resource sandboxes require `resource_id`/`--resource`. HTTP exec defaults to live NDJSON (`stream=true` is explicit; `stream=false` selects buffered JSON); the CLI defaults to live human-readable output, with `--events` for NDJSON and `--buffered` for one final response. All modes retain bounded output and timeout enforcement; interactive stdin/PTY is intentionally out of scope. See [ADR-0008](docs/adr/0008-streaming-command-execution.md) for the status-code and terminal-event contract, and [the streaming-first design](docs/superpowers/specs/2026-09-01-streaming-first-exec-design.md) for the default-mode decision. Same-resource concurrency/queueing remains a follow-up to #260.
