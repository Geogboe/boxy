...
- **`pkg/agentsdk` (live) vs. a config-declared `agents:` list (removed, 2026-07)**: don't confuse these. `pkg/agentsdk.EmbeddedAgent` is real, in-process code wired into `boxy serve` today. A separate `Config.Agents`/`AgentSpec` field once existed for a *pull-model* remote agent (server dials out to a static agent address) but was dead code — never read anywhere — and has been deleted. The actual planned remote-agent design is a *push* model (agent dials the server over gRPC) and doesn't exist yet; see #37/#62. The full transport/TLS/registration design is now decided (not yet implemented) in [ADR-0005](docs/adr/0005-remote-agent-transport-and-registration.md) — gRPC bidirectional streaming, a boxy-owned private CA with full mTLS, single-use bootstrap tokens exchanged for a client cert, and per-resource agent provenance (`ProviderRef.AgentID`) so `Destroy`/`Allocate` route to the exact agent that created a resource rather than any agent offering the same provider type.
